...
class Camion(object):
    ...
    def __init__(self, entorno, nombre):
        ...
        self.trailer = MedioDeAlmacenamiento(entorno, str(self.nombre), 1, niveles, 28, self.peso)
    ...
    def espera_operacion(self, entorno, operacion, medio_de_origen_o_destino, tpaciencia_r, tpaciencia_p):
        ...
        if self.tipo == "Carga":
            ...
            while espera_t < tpaciencia_t:
                ...
                if self.nombre == 71:
                    print str(espera_m_od) + " " + str(espera_r) + " " + str(espera_p)

                if self in medio_de_origen_o_destino.c_esp_disp_at_alm[0:medio_de_origen_o_destino.cap_atencion]:

                    # TODO verificar y mejorar
                    if self in operacion.recurso.camiones_esperando_carga[0:operacion.recurso.capacity]:
                        ...
                        if self.transbordo == "Si" \
                            ...
                            if any(c.verifica_almacenes(entorno.recursos) and
                                   c in operacion.recurso.camiones_esperando_carga[
                                            operacion.recurso.camiones_esperando_carga.index(self) + 1:]
                                   for c in medio_de_origen_o_destino.c_esp_disp_at_alm[
                                            medio_de_origen_o_destino.c_esp_disp_at_alm.index(self) + 1:]):
                                c_adelantado = [ca for ca in medio_de_origen_o_destino.c_esp_disp_at_alm[
                                                             medio_de_origen_o_destino.c_esp_disp_at_alm.index(
                                                                 self) + 1:]
                                                if ca.verifica_almacenes(entorno.recursos) and
                                                ca in operacion.recurso.camiones_esperando_carga[
                                                          operacion.recurso.camiones_esperando_carga.index(
                                                              self) + 1:]][0]

                                self.adelanta_camion(
                                    entorno, operacion, medio_de_origen_o_destino, c_adelantado, "Almacen")

                                c_adelantado = None

                            # Identificar si se puede adelantar algun camion
                            if any(c.verifica_almacenes(entorno.recursos) and
                                   c.verifica_colas_recursos_m(entorno.recursos)
                                   for c in medio_de_origen_o_destino.c_esp_disp_at_alm[
                                            medio_de_origen_o_destino.c_esp_disp_at_alm.index(self) + 1:]):
                                c_adelantado = [ca for ca in medio_de_origen_o_destino.c_esp_disp_at_alm[
                                                             medio_de_origen_o_destino.c_esp_disp_at_alm.index(
                                                                 self) + 1:]
                                                if ca.verifica_almacenes(entorno.recursos) and
                                                ca.verifica_colas_recursos_m(entorno.recursos)][0]
                            ...
                            if any(c.verifica_almacenes(entorno.recursos) and
                                   c.verifica_colas_almacenes(entorno.recursos)  # TODO verificar
                                ...
                                c_adelantado = [ca for ca in operacion.recurso.camiones_esperando_carga[
                                                             operacion.recurso.camiones_esperando_carga.index(
                                                                 self) + 1:]
                                                if ca.verifica_almacenes(entorno.recursos) and
                                                ca.verifica_colas_almacenes(entorno.recursos)][0]

                                self.adelanta_camion(
                                    entorno, operacion, medio_de_origen_o_destino, c_adelantado, "Operacion")
                        ...
                        if self.verifica_almacenes(entorno.recursos):
                            espera_r += 1
                        else:
                            espera_r += 1
                            espera_p += 1
                    ...
                    # TODO verificar y mejorar
                    if self in operacion.recurso.camiones_esperando_carga[0:operacion.recurso.capacity] and \
                            any(c.verifica_almacenes(entorno.recursos) and
                                c.verifica_colas_almacenes(entorno.recursos)  # TODO verificar
                                for c in operacion.recurso.camiones_esperando_carga[
                                         operacion.recurso.camiones_esperando_carga.index(self) + 1:]) \
                            and c_adelantado is None:
                        ...
                        self.adelanta_camion(entorno, operacion, medio_de_origen_o_destino, c_adelantado, "Operacion")
            ...
            if (self.transbordo == "Si" or medio_de_origen_o_destino.niveles[self.carga] >= 28 * (1 + num_c_iguales)) \
                    and espera_t <= tpaciencia_t \
                    and self in operacion.recurso.camiones_esperando_carga[0:operacion.recurso.capacity] \
                    and self in medio_de_origen_o_destino.c_esp_disp_at_alm[0:medio_de_origen_o_destino.cap_atencion]:

                entorno.exit(["Inicia", espera_r, espera_p, espera_m_od, espera_t])
                ...
                entorno.exit(["No inicia", espera_r, espera_p, espera_m_od])
                    ...
                    if self in operacion.recurso.camiones_esperando_carga[0:operacion.recurso.capacity]:
                        ...
                        if self.transbordo == "Si" \
                            ...
                            if any(c.verifica_almacenes(entorno.recursos) and
                                   ...
                                   for c in operacion.recurso.camiones_esperando_carga[
                                            operacion.recurso.camiones_esperando_carga.index(
                                                self) + 1:]) and c_adelantado is None:
                                c_adelantado = \
            ...
            if (self.transbordo == "Si" or espacio >= 28 * (1 + num_c_iguales)) and espera_t <= tpaciencia_t \
                    and self in operacion.recurso.camiones_esperando_carga[0:operacion.recurso.capacity] \
                    and self in medio_de_origen_o_destino.c_esp_disp_at_alm[0:medio_de_origen_o_destino.cap_atencion]:
                entorno.exit(["Inicia", espera_r, espera_p, espera_m_od, espera_t])
                print operacion.recurso.camiones_esperando_carga
    ...
    def solicita_adelanto(self, entorno, operacion, medio_de_origen_o_destino):
        ...
        primeros_camiones_en_cola_op = operacion.recurso.camiones_esperando_carga[0:operacion.recurso.capacity]
        primeros_camiones_en_cola_m_od = medio_de_origen_o_destino.c_esp_disp_at_alm[
                                         0:medio_de_origen_o_destino.cap_atencion]

        prod_o_esp_disp_primeros_camiones_en_cola_op = all(camion.verifica_almacenes(entorno.recursos)
                                                           for camion in primeros_camiones_en_cola_op)

        prod_o_esp_disp_primeros_camiones_en_cola_m_od = all(camion.verifica_almacenes(entorno.recursos)
                                                             for camion in primeros_camiones_en_cola_m_od)

        primeros_camiones_en_cola_primeros_en_colas_m_od = all(camion.verifica_colas_almacenes(entorno.recursos)
                                                               for camion in primeros_camiones_en_cola_op)

        t_estaciones_disp_rec = operacion.recurso.capacity - operacion.recurso.count

        if operacion.nombre == "Transbordo en sistema mecanizado":
            pass
        elif self.tipo == "Descarga" \
                and medio_de_origen_o_destino.capacity - medio_de_origen_o_destino.level >= 28 * (1 + num_c_iguales):

            if self in primeros_camiones_en_cola_m_od and not prod_o_esp_disp_primeros_camiones_en_cola_op:
                primeros_camiones_en_cola_op[operacion.recurso.capacity - t_estaciones_disp_rec].adelanta_camion(
                    entorno, operacion, medio_de_origen_o_destino, self, "Operacion")

            elif self in primeros_camiones_en_cola_m_od and not primeros_camiones_en_cola_primeros_en_colas_m_od:
                primeros_camiones_en_cola_op[operacion.recurso.capacity - t_estaciones_disp_rec].adelanta_camion(
                    entorno, operacion, medio_de_origen_o_destino, self, "Operacion")
                print "**********PRUEBA A"

            if self in primeros_camiones_en_cola_op and not prod_o_esp_disp_primeros_camiones_en_cola_m_od:
                print medio_de_origen_o_destino.estaciones_en_uso
                primeros_camiones_en_cola_m_od[medio_de_origen_o_destino.estaciones_en_uso].adelanta_camion(
                    entorno, operacion, medio_de_origen_o_destino, self, "Almacen")
                print "**********PRUEBA B"

        elif self.tipo == "Carga" \
                and medio_de_origen_o_destino.niveles[self.carga] >= 28 * (1 + num_c_iguales):
            ...
            if self in primeros_camiones_en_cola_op and not prod_o_esp_disp_primeros_camiones_en_cola_m_od:
                ...
                and not prod_o_esp_disp_primeros_camiones_en_cola_op:
    ...
    def adelanta_camion(self, entorno, operacion, medio_de_origen_o_destino, camion, tipo):
        ...
        if tipo == "Operacion":
            ...
            if self in medio_de_origen_o_destino.c_esp_disp_at_alm \
                    and camion in medio_de_origen_o_destino.c_esp_disp_at_alm:
                medio_de_origen_o_destino.c_esp_disp_at_alm.remove(camion)
                medio_de_origen_o_destino.c_esp_disp_at_alm = medio_de_origen_o_destino.c_esp_disp_at_alm[
                                                              0:medio_de_origen_o_destino.c_esp_disp_at_alm.index(
                                                                  self)] + [camion] \
                    + medio_de_origen_o_destino.c_esp_disp_at_alm[
                      medio_de_origen_o_destino.c_esp_disp_at_alm.index(self):]
                print "\t" + medio_de_origen_o_destino.nombre + ":" \
                      + str(medio_de_origen_o_destino.c_esp_disp_at_alm) + " Hora: " + str(entorno.now)

        elif tipo == "Almacen":

            medio_de_origen_o_destino.c_esp_disp_at_alm.remove(camion)
            medio_de_origen_o_destino.c_esp_disp_at_alm = \
                medio_de_origen_o_destino.c_esp_disp_at_alm[0:medio_de_origen_o_destino.c_esp_disp_at_alm.index(self)] \
                + [camion] \
                + medio_de_origen_o_destino.c_esp_disp_at_alm[medio_de_origen_o_destino.c_esp_disp_at_alm.index(self):]
    ...
    def verifica_almacenes(self, recursos):
        ...
        for almacen in almacenes:
            ...
            if self in almacen.c_esp_disp_at_alm and self.tipo == "Carga":
                num_c_iguales += sum(1 for c in almacen.c_esp_disp_at_alm[0:almacen.c_esp_disp_at_alm.index(self)]
                                     if c.carga == self.carga and c.tipo == self.tipo and c.transbordo == "No")
            elif self not in almacen.c_esp_disp_at_alm and self.tipo == "Carga":
                num_c_iguales += sum(1 for c in almacen.c_esp_disp_at_alm
                                     if c.carga == self.carga and c.tipo == self.tipo and c.transbordo == "No")
            elif self in almacen.c_esp_disp_at_alm and self.tipo == "Descarga":
                num_c_iguales += sum(1 for c in almacen.c_esp_disp_at_alm[0:almacen.c_esp_disp_at_alm.index(self)]
                                     if c.tipo == self.tipo and c.transbordo == "No")
            elif self not in almacen.c_esp_disp_at_alm and self.tipo == "Descarga":
                num_c_iguales += sum(1 for c in almacen.c_esp_disp_at_alm
                                     if c.tipo == self.tipo and c.transbordo == "No")
            else:
                print "********** WTF!!"
        ...
        if self.transbordo == "Si":
            prod_o_esp_dip = True
                ...
                and any(self.carga in almacen.niveles.keys() and almacen.niveles[self.carga] >= 28 * (1 + num_c_iguales)
                        for almacen in almacenes):
            prod_o_esp_dip = True
        elif self.tipo == "Descarga" \
    ...
    def verifica_colas_almacenes(self, recursos):
        almacenes = [recursos["Almacen 2"], recursos["Almacen 1"],
                     recursos["Almacen Ext"], recursos["Patio de Contenedores"],
                     recursos["Tanque 1"]]
        ...
        if self.transbordo == "Si":
            entre_primeros = True
        elif any(self.carga in almacen.niveles.keys() and self in almacen.c_esp_disp_at_alm[0:almacen.cap_atencion]
                 for almacen in almacenes):
            entre_primeros = True
        return entre_primeros

    def verifica_colas_recursos_m(self, recursos):
        entre_primeros = False
        recursos_m = [
        ...
        if any(self in recurso.camiones_esperando_carga[0:recurso.capacity]
               for recurso in recursos_m):
            entre_primeros = True
        return entre_primeros
...
class OperacionManipuleo(Operacion):
    ...
    def ejecutar(self, entorno, camion, tpaciencia_r=None, tpaciencia_p=None, medio_de_origen_o_destino=None):
        ...
        if medio_de_origen_o_destino is not None:
            ...
            medio_de_origen_o_destino.c_esp_disp_at_alm.append(camion)
            ...
            if espera_producto[0] == "Inicia":
                ...
                espera_m_od = espera_producto[3]
                espera_t = espera_producto[4]
                ...
                with self.recurso.request() as turno:
                    ...
                    if turno.triggered:
                        ...
                        medio_de_origen_o_destino.estaciones_en_uso += 1
                        yield atencion
                        ...
                        # TODO verificar y mejorar
                        # if self.nombre in ["Carga a pulso - Sacos", "Descarga a pulso - Sacos"]:
                        medio_de_origen_o_destino.c_esp_disp_at_alm.remove(camion)
                ...
                # TODO verificar y mejorar
                # if self.nombre in ["Carga a pulso - Sacos", "Descarga a pulso - Sacos"]:
                print medio_de_origen_o_destino.c_esp_disp_at_alm
                print camion
                medio_de_origen_o_destino.c_esp_disp_at_alm.remove(camion)
...
class MedioDeAlmacenamiento(simpy.Container):
    def __init__(self, env, nombre, cap_atencion, niveles=None, capacity=float('inf'), init=0):
        """
        ...
        self.nombre = nombre
        self.cap_atencion = cap_atencion
        self.estaciones_en_uso = 0
        self.c_esp_disp_at_alm = []
